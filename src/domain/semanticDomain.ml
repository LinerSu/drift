...
module SemanticsDomain = struct
  ...
  let alpha_rename_R (a : relation_t) prevar var : relation_t =
    ...
    | Bool (vt, vf) ->
        Bool
          ( AbstractValue.alpha_rename vt prevar var
          , AbstractValue.alpha_rename vf prevar var )
    | Unit _ -> a

  let type_top_R (typ : inputType) =
    match typ with
    | Int -> Int AbstractValue.top
    | Bool -> Bool (AbstractValue.top, AbstractValue.top)
    | Unit -> Unit ()

  let type_bot_R (typ : inputType) =
    match typ with
  ...
  let top_R = function
    ...
    | Ge | Eq | Ne | Lt | Gt | Le | And | Or ->
        Bool (AbstractValue.top, AbstractValue.top)
    | _ ->
        raise (Invalid_argument "top_R should use a relational type operator")
  ...
  let bot_R = function
    ...
    | Ge | Eq | Ne | Lt | Gt | Le | And | Or ->
        Bool (AbstractValue.bot, AbstractValue.bot)
    | _ ->
        raise (Invalid_argument "bot_R should use a relational type operator")

  let is_bool_R a = match a with Bool _ -> true | _ -> false

  let init_R_c (c : value) =
    match c with
    ...
    | UnitLit -> Unit ()
    | IntList lst ->
        raise (Invalid_argument "This should be cover on the upper level")

  let join_R a1 a2 =
    match (a1, a2) with
    | Int v1, Int v2 -> Int (AbstractValue.join v1 v2)
    | Bool (v1t, v1f), Bool (v2t, v2f) ->
        Bool (AbstractValue.join v1t v2t, AbstractValue.join v1f v2f)
    | Unit _, a | a, Unit _ -> a
  ...
  let meet_R a1 a2 =
    ...
    | Bool (v1t, v1f), Bool (v2t, v2f) ->
        Bool (AbstractValue.meet v1t v2t, AbstractValue.meet v1f v2f)
  ...
  let leq_R a1 a2 =
    ...
    | Bool (v1t, v1f), Bool (v2t, v2f) ->
        AbstractValue.leq v1t v2t && AbstractValue.leq v1f v2f
  ...
  let eq_R a1 a2 =
    ...
    | Bool (v1t, v1f), Bool (v2t, v2f) ->
        AbstractValue.eq v1t v2t && AbstractValue.eq v1f v2f
    | Unit u1, Unit u2 -> true
    | _, _ -> false
  ...
  let arrow_R var a1 a2 =
    ...
    | Bool (vt1, vf1), Bool (vt2, vf2) ->
        ...
        let vt1' =
          AbstractValue.join
            (AbstractValue.meet vt1 vt2)
            (AbstractValue.meet vt1 vf2)
        in
        let vf1' =
          AbstractValue.join
            (AbstractValue.meet vf1 vt2)
            (AbstractValue.meet vf1 vf2)
        in
        Bool (vt1', vf1')
    | Int v, Bool (vt, vf) ->
        (* {v:int| a^at V a^af} *)
        Int
          (AbstractValue.join (AbstractValue.meet v vt)
             (AbstractValue.meet v vf) )
  ...
  let forget_R var a =
    ...
    | Bool (vt, vf) ->
        Bool
          (AbstractValue.forget_var var vt, AbstractValue.forget_var var vf)
    | Unit _ -> a
  ...
  let equal_R a var =
    let eq_a =
      ...
      | Bool (vt, vf) ->
          Bool
            ( AbstractValue.equal_var vt "cur_v" var
            , AbstractValue.equal_var vf "cur_v" var )
      | Unit _ -> a
    in
    eq_a
  ...
  let wid_R a1 a2 =
    ...
    | Bool (v1t, v1f), Bool (v2t, v2f) ->
        Bool (AbstractValue.widening v1t v2t, AbstractValue.widening v1f v2f)
  ...
  let sat_equal_R a x =
    ...
    | Bool (vt, vf) ->
        AbstractValue.sat_cons vt x && AbstractValue.sat_cons vf x
    | _ -> false

  let op_R res l r op cons a =
    (*cons for flag of linear constraints*)
    match op with
    ...
    | Ge | Eq | Ne | Lt | Gt | Le -> (
        if cons then
          ...
          | Bool (vt, vf) ->
              Bool
                ( AbstractValue.operator res l r op 1 vt
                , AbstractValue.operator res l r (rev_op op) 0 vf )
          | Unit _ -> raise (Invalid_argument "opR: Given a unit type")
        else
          match a with
          | Int v ->
              Bool
                ( AbstractValue.operator res l r op 1 v
                , AbstractValue.operator res l r (rev_op op) 0 v )
          | Bool (vt, vf) ->
              Bool
                ( AbstractValue.operator res l r op 1 vt
                , AbstractValue.operator res l r (rev_op op) 0 vf )
    ...
    | Cons | Seq | And | Or ->
        raise
          (Invalid_argument ("Invalid operator matched " ^ string_of_op op))

  let uop_R res op e cons a =
    (*cons for flag of linear constraints*)
    match op with
    | UMinus -> (
      match a with
  ...
  let bool_op_R op a1 a2 =
    match (a1, a2) with
    | Bool (v1t, v1f), Bool (v2t, v2f) ->
        if string_of_op op = "&&" then
          Bool (AbstractValue.meet v1t v2t, AbstractValue.join v1f v2f)
        else Bool (AbstractValue.join v1t v2t, AbstractValue.meet v1f v2f)
    | _, _ ->
        raise
          (Invalid_argument "&& or || operation: Base Type should be bool")

  let replace_R a var x = alpha_rename_R a var x

  let extrac_bool_R v b =
    match (v, b) with
    ...
    | _, _ ->
        raise
          (Invalid_argument
             "Extract abstract value for condition, expect bool one" )

  let stren_R a ae =
    match (a, ae) with
    | Int v, Int vae -> Int (AbstractValue.meet v vae)
    | Bool (vt, vf), Int vae ->
        Bool (AbstractValue.meet vt vae, AbstractValue.meet vf vae)
    | Unit _, Int vae -> a
  ...
  let der_R exp a =
    match a with
    | Bool (vt, vf) ->
        Bool (AbstractValue.derived exp vt, AbstractValue.derived exp vf)
  ...
  let proj_R a vars =
    ...
    | Bool (vt, vf) ->
        Bool
          ( AbstractValue.project_other_vars vt vars
          , AbstractValue.project_other_vars vf vars )
    | Unit _ -> a
  ...
  let is_bool_false_R a =
    match a with
    | Bool (vt, vf) ->
        AbstractValue.is_bot vf && AbstractValue.is_bot vt <> true
    | _ -> raise (Invalid_argument "Expect a bool value")

  let opt_eq_R a1 a2 =
    is_bot_R a1 = false && is_bot_R a2 = false && eq_R a1 a2

  let contains_var_R var a =
    match a with
    | Int v -> AbstractValue.contains_var var v
    | Bool (vt, vf) ->
        AbstractValue.contains_var var vt
        && AbstractValue.contains_var var vf
    | Unit _ -> false
  ...
  let is_unit_R = function Unit _ -> true | _ -> false
  ...
  (* ******************************** ** Abstract domain for Values **
     ******************************** *)
  let rec alpha_rename_V v prevar var =
    match v with
  ...
  and init_V_c = function
    | IntList lst ->
        if List.length lst = 0 then Lst (init_Lst_c ())
  ...
  and join_V (v1 : value_t) (v2 : value_t) : value_t =
    match (v1, v2) with
    | Bot, v | v, Bot -> v
  ...
  and meet_V (v1 : value_t) (v2 : value_t) : value_t =
    match (v1, v2) with
    | Top, v | v, Top -> v
  ...
  and leq_V (v1 : value_t) (v2 : value_t) : bool =
    match (v1, v2) with
    | Bot, _ -> true
    | _, Top -> true
  ...
  and sat_leq_V (v1 : value_t) (v2 : value_t) : bool =
    match (v1, v2) with
    | Bot, _ -> true
    | _, Top -> true
    | Relation r1, Relation r2 -> leq_R r1 r2
  ...
  and eq_V (v1 : value_t) (v2 : value_t) : bool =
    match (v1, v2) with
    | Bot, Bot -> true
    | Top, Top -> true
  ...
  and is_table (v : value_t) = match v with Table _ -> true | _ -> false

  and is_bool_V (v : value_t) =
    match v with Relation r -> is_bool_R r | _ -> false

  and is_Relation = function Relation _ -> true | _ -> false

  and is_Array = function Ary _ -> true | _ -> false

  and is_List = function Lst _ -> true | _ -> false

  and arrow_V var (v : value_t) (v' : value_t) =
    match v' with
    | Bot | Top | Table _ -> v
    | Relation r2 -> (
      match v with
      ...
      | _ -> v )
    | Tuple u2 -> List.fold_left (fun v1 v2 -> arrow_V var v1 v2) v u2
    | Variant vart2 ->
        let _, u = vart2 in
        arrow_V var v (Tuple u)
    | Ref (v2, own2) -> arrow_V var v v2
    | Ary ary2 -> (
        let vars, (rl2, re2) = ary2 in
        match v with
        | Table t -> Table (arrow_T forget_V arrow_V var t v')
        | Relation r1 ->
            Relation
              (let r1' = arrow_R var r1 rl2 in
               if is_bot_R re2 then r1' else arrow_R var r1' re2 )
        | Ary ary -> Ary (arrow_Ary var ary re2 (Some rl2))
        | Lst lst ->
            Lst (arrow_Lst var lst (Relation re2) (Some (vars, rl2)))
        | _ -> v )
    | Lst lst2 -> (
        let ((l2, e2) as vars), (rl2, ve2) = lst2 in
        match v with
        | Table t -> Table (arrow_T forget_V arrow_V var t v')
        | Relation r1 -> (
            if is_bot_R rl2 then Relation (bot_shape_R r1)
            else
              let r1' =
                let res = meet_R r1 rl2 in
                if is_bot_R res then meet_R (forget_R l2 r1) rl2 else res
              in
              match ve2 with
              | Bot ->
                  if String.length var >= 2 && String.sub var 0 2 = "zh" then
  ...
  and op_V sl sr op v =
    ...
    | _ ->
        raise (Invalid_argument "Should be a relation type when using op_V")
  ...
  and uop_V op s v =
    ...
    | _ ->
        raise (Invalid_argument "Should be a relation type when using uop_V")

  and bool_op_V op v1 v2 =
    match (v1, v2) with
    ...
    | _, _ ->
        raise
          (Invalid_argument "Should be a relation type when using bool_op_V")

  and is_Bot_V = function Bot -> true | _ -> false

  and replace_V v var x =
    match v with
  ...
  and extrac_bool_V v b =
    match v with
    | Relation r -> Relation (extrac_bool_R r b)
    | _ ->
        raise (Invalid_argument "Should be relation when split if statement")

  and stren_V v ae =
    match (v, ae) with
    | Bot, _ -> Bot
    | Relation r, Relation rae ->
        if is_bot_R rae && is_unit_R r then Bot else Relation (stren_R r rae)
  ...
  and sat_equal_V v x =
    match v with Relation r -> sat_equal_R r x | _ -> false

  (* and der_V term v = match term, v with | _, Top | _, Bot -> v | Const
     (c,l), Relation r -> let r' = (match c with | Integer i -> der_R
     ((string_of_int i) ^ "=" ^ (name_of_node l)) r | Boolean b -> r
     (*TODO:this case*)) in Relation r' | Var (x, l), Relation r -> Relation
     (der_R (x ^ "=" ^ (name_of_node l)) r) | App (e1, e2, l), Relation r ->
     v |> der_V e1 |> der_V e2 | Rec (f_opt, x, lx, e1, l), Table t -> v
     (*TODO:this case*) | Ite (e1, e2, e3, l, _), Relation r -> v |> der_V e1
     |> der_V e2 |> der_V e3 | BinOp (bop, e1, e2, l), Relation r -> let expr
     = ((e1 |> loc |> name_of_node)^(string_of_op bop)^(e2 |> loc |>
     name_of_node) ^ "=" ^ (name_of_node l)) in let v' = Relation (der_R expr
     r) in v' |> der_V e1 |> der_V e2 | _, _ -> raise (Invalid_argument
     "derived values match incorrectly") *)
  and proj_V v vars =
    match v with
  ...
  (* and opt_eq_V v1 v2 = match v1, v2 with | Bot, _ | _, Bot -> false | _,
     Top -> true | Relation r1, Relation r2 -> is_bot_R r1 = false &&
     is_bot_R r2 = false && eq_R r1 r2 | Table (z1, v1i, v1o), Table (z2,
     v2i, v2o) -> z1 = z2 && opt_eq_V v1i v2i && opt_eq_V v1o v2o | Ary ary1,
     Ary ary2 -> eq_Ary ary1 ary2 | Unit u1, Unit u2 -> true | _, _ -> false *)
  and get_relation_V = function
    | Relation r -> r
    | _ -> raise (Invalid_argument "expect relation type when extract value")

  and is_bool_bot_V = function Relation r -> is_bool_bot_R r | _ -> true
        ...
        if table_isempty t then Bot
          ...
          | Table t ->
              if table_isempty t then Bot
                ...
                let _, (vi, _) = get_full_table_T t in
                vi
          | _ -> raise (Invalid_argument "array.set should be a table") )
    | _ -> raise (Invalid_argument "array.set should be a table")

  and join_for_item_V v1 v2 =
    match (v1, v2) with
    | Bot, _ | _, Bot -> v1
    ...
    | _, _ ->
        raise
          (Invalid_argument
             "Approximate array's item should use relation type" )
  ...
  and list_cons_V f v1 v2 =
    match (v1, v2) with
    | Bot, _ | _, Bot -> Bot
    | v, Lst lst -> Lst (list_cons_Lst f v lst)
    | _, _ -> raise (Invalid_argument "List construct should be item :: lst")

  and alpha_rename_Vs v1 v2 =
    match (v1, v2) with
    | ( Lst (((l1, e1), (rl1, ve1)) as lst1)
      , Lst (((l2, e2), (rl2, ve2)) as lst2) ) ->
        if l1 = l2 && e1 = e2 then (Lst lst1, Lst lst2)
        else
          let lst2 =
            ( (l2, e2)
            , ( rl2 |> forget_R l1 |> forget_R e1
              , ve2 |> forget_V l1 |> forget_V e1 ) )
          in
  ...
  and bot_relation_V (tp : inputType) =
    match tp with
  ...
  and cons_temp_lst_V t = function
    ...
    | _ ->
        raise (Invalid_argument "Temp list construct should be item :: lst")

  and item_shape_V te t =
    match (te, t) with
    | Lst lste, Lst lst -> Lst (item_shape_Lst lste lst)
    | _, _ -> t
  ...
  and rename_lambda_V v =
    match v with Lst lst -> Lst (rename_lambda_Lst lst) | _ -> v
  ...
  and join_Ary ary1 ary2 =
    let ary1', ary2' = alpha_rename_Arys ary1 ary2 in
    let vars1, (rl1, re1) = ary1' in
    let vars2, (rl2, re2) = ary2' in
  ...
  and leq_Ary ary1 ary2 =
    let (l1, e1), (rl1, re1) = ary1 in
    let (l2, e2), (rl2, re2) = ary2 in
  ...
  and arrow_Ary var ary r ropt =
    let ((l, e) as vars), (rl, re) = ary in
  ...
  and alpha_rename_Arys ary1 ary2 =
    ...
    let (l2, e2), (rl2, re2) = ary2 in
    let ary2' =
      match (l1 = l2, e1 = e2) with
      | true, true -> ary2
      | false, true -> alpha_rename_Ary ary2 l2 l1
      | true, false -> alpha_rename_Ary ary2 e2 e1
      | false, false ->
          let ary2 = alpha_rename_Ary ary2 l2 l1 in
          alpha_rename_Ary ary2 e2 e1
    in
    (ary1, ary2')

  and alpha_rename_Ary ary prevar var =
    let (l, e), (rl, re) = ary in
  ...
  and join_for_item_Ary ary r =
    let (l, e), (rl, re) = ary in
    if is_bot_R rl && is_bot_R re then ary
    else
      let re' = alpha_rename_R r "cur_v" e |> join_R re in
      ((l, e), (rl, re'))

  and bot_shape_Ary (vars, (rl, re)) = (vars, (rl, bot_shape_R re))

  and only_shape_Ary (vars, (rl, re)) = is_bot_R rl && is_bot_R re

  (* ******************************* ** Abstract domain for List **
     ******************************* *)
  and init_Lst_c () : list_t =
    let varl, vare = (fresh_length (), fresh_item ()) in
  ...
  and const_Lst lst =
    ...
    in
    let rl =
      top_R Plus |> op_R varl varl (string_of_int (List.length lst)) Eq true
    in
    let re =
      rl
      |> op_R vare vare (string_of_int min) Ge true
      |> op_R vare vare (string_of_int max) Le true
    in
    ((varl, vare), (rl, Relation re))

  and init_Lst vars : list_t =
    let varl, vare = vars in
    let varl' =
      if varl = "l" || varl = "l'" then fresh_length () else varl
    in
  ...
  and extrac_item_Lst vars ((_, vare), (_, ve)) =
    ...
    let ve' =
      match ve with Relation re -> alpha_rename_V ve vare "cur_v" | _ -> ve
  ...
  and join_Lst lst1 lst2 =
    ...
    let lst1', lst2' =
      if l1 <> "l" && contains_var_R l1 rl2 then
        let a, b = alpha_rename_Lsts lst2 lst1 in
        (b, a)
      else alpha_rename_Lsts lst1 lst2
    in
    let ((l1, e1) as vars1), (rl1, ve1) = lst1' in
    let ((l2, e2) as vars2), (rl2, ve2) = lst2' in
    (vars1, (join_R rl1 rl2, join_V ve1 ve2))

  and meet_Lst lst1 lst2 =
    let lst1', lst2' = alpha_rename_Lsts lst1 lst2 in
    let vars1, (rl1, ve1) = lst1' in
    let vars2, (rl2, ve2) = lst2' in
  ...
  and arrow_Lst var lst v ropt =
    ...
    | Some ((_, e'), rl') -> (
      match (v, ve) with
      | Bot, _ ->
          if String.sub var 0 2 = "xs" && contains_var_R "zc" rl' then
      ...
      | _, _ ->
          if String.sub var 0 2 = "xs" then
            ...
            let ve' =
              if is_Bot_V v then
                (* TODO: how could we get abstract value without change
                   transformer *)
                ve
              else
  ...
  and alpha_rename_Lsts lst1 lst2 =
    ...
    let lst2' =
      ...
      | false, false ->
          let lst2 = alpha_rename_Lst lst2 l2 l1 in
          alpha_rename_Lst lst2 e2 e1
    in
    (lst1, lst2')

  and alpha_rename_Lst lst prevar var =
    let (l, e), (rl, ve) = lst in
    let l' = if l = prevar then var else l in
    let e' = if e = prevar then var else e in
    let rl' = alpha_rename_R rl prevar var in
  ...
  and reduce_len_Lst len le_lst lst =
    let (l, e), (rl, ve) = lst in
    let l', e' =
      match le_lst with
      | [] -> (fresh_length (), fresh_item ())
      | [l'; e'] ->
          if l = l' then (fresh_length (), fresh_item ()) else (l', e')
      | _ ->
          raise
            (Invalid_argument "construct pattern tl should give [] or [l;e]")
    in
    let rl' =
      assign_R l' l (string_of_int len) Minus rl |> op_R "" l' "0" Ge true
  ...
  and list_cons_Lst f v lst =
    let (l, e), (rl, ve) = lst in
    if v = Bot || is_bot_R rl then ((l, e), (bot_R Plus, Bot))
    else
      (* let v = stren_V v (Relation (forget_R l rl)) in *)
      let rl' = assign_R l l "1" Plus rl in
      let ve' =
        match (v, ve) with
        | Relation r, Bot -> Relation (arrow_R e rl' r)
        | Relation r, Relation re ->
            let re' =
              arrow_R e rl' (forget_R l r)
              |> join_R (meet_R (forget_R l re) rl')
            in
            Relation re'
        | Table t, Bot ->
            let t' = init_T (dx_T v) in
            Table t'
        | Tuple u, Bot ->
  ...
  and only_shape_Lst ((l, e), (rl, ve)) = is_bot_R rl && ve = Bot

  and prop_Lst prop (((l1, e1) as vars1), (rl1, ve1))
      (((l2, e2) as vars2), (rl2, ve2)) =
    ...
    let ve1', ve2' =
      ...
      | _, _ -> (ve1, join_V ve1 ve2)
    in
    ((vars1, (rl1', ve1')), (vars2, (rl2', ve2')))

  and cons_temp_lst_Lst v (((l, e) as vars), (rl, ve)) =
    let ve' =
      if is_bot_R rl then bot_shape_V ve
      else
        match v with
        | Relation r ->
            let re' = alpha_rename_R r "cur_v" e in
            Relation re'
        | _ -> v
    in
    (vars, (rl, ve'))
  ...
  and bot_shape_Lst (vars, (rl, ve)) = (vars, (rl, bot_shape_V ve))

  (* ******************************* ** Abstract domain for Tuple **
     ******************************* *)
  and join_Tuple u1 u2 = List.map2 (fun v1 v2 -> join_V v1 v2) u1 u2

  and meet_Tuple u1 u2 = List.map2 (fun v1 v2 -> meet_V v1 v2) u1 u2
  ...
  and wid_Tuple u1 u2 = List.map2 (fun v1 v2 -> wid_V v1 v2) u1 u2

  and arrow_Tuple var u v' = List.map (fun v -> arrow_V var v v') u

  and stren_Tuple u ae = List.map (fun v -> stren_V v ae) u

  and add_tuple_item_Tuple u v = v :: u

  and proj_Tuple u vars = List.map (fun v -> proj_V v vars) u

  and forget_Tuple var u = List.map (fun v -> forget_V var v) u

  and bot_shape_Tuple u = List.map (fun v -> bot_shape_V v) u

  and get_tuple_list u = u

  and replace_Tuple u var x = List.map (fun v -> replace_V v var x) u

  and is_Bot_Tuple u = List.fold_left (fun ret v -> ret && is_Bot_V v) true u
  ...
  and join_Variant var1 var2 =
    let tag1, u1 = var1 in
    let tag2, u2 = var2 in
    if tag1 = tag2 then (tag1, join_Tuple u1 u2) else var1
    (* TODO: DEAD END *)

  and meet_Variant var1 var2 =
    let tag1, u1 = var1 in
    let tag2, u2 = var2 in
    if tag1 = tag2 then (tag1, meet_Tuple u1 u2) else var1
    (* TODO: DEAD END *)

  and leq_Variant var1 var2 =
    let tag1, u1 = var1 in
    let tag2, u2 = var2 in
    if tag1 = tag2 then leq_Tuple u1 u2 else false

  and eq_Variant var1 var2 =
    let tag1, u1 = var1 in
    let tag2, u2 = var2 in
    if tag1 = tag2 then eq_Tuple u1 u2 else false

  and wid_Variant var1 var2 =
    let tag1, u1 = var1 in
    let tag2, u2 = var2 in
    if tag1 = tag2 then (tag1, wid_Tuple u1 u2) else var1
    (* TODO: DEAD END *)
            ...
            let v =
              let cons_v usey =
                ...
                | Exp mt ->
                    let rec cons_val mt =
                      ...
                      | Single t ->
                          ...
                          let org_r =
                            if usey then
                              join_V
                                (alpha_rename_V v "cur_v" dp_vary)
                                (alpha_rename_V v dp_varx dp_vary)
                              |> get_relation_V
                            else top_R Plus
                          in
                          Relation
                            ( org_r
                            |> constrcut_type_pred_R exp
                                 (VarSet.elements var_lst) )
                      | Or (t, tmt) ->
                          let vlst = cons_val tmt in
                          let exp, var_lst = term_to_str VarSet.empty t in
                          let org_r =
                            if usey then
                              alpha_rename_V v dp_varx dp_vary
                    ...
                    cons_val mt
              in
              if v = Top then cons_v false else cons_v true
            in
            v :: lst )
          [] super_ref_lst
        |> List.rev |> join_Tuple ret )
      ret idx_lst

  (* *********************************** ** Abstract domain for Reference **
     *********************************** *)
  and join_Ref ((v1, own1) as ref1) (v2, own2) =
    if own1 = own2 then (join_V v1 v2, own1) else ref1
    (* TODO: if two ownership not equal, return what *)

  and meet_Ref ((v1, own1) as ref1) (v2, own2) =
    if own1 = own2 then (meet_V v1 v2, own1) else ref1
    (* TODO: if two ownership not equal, return what *)

  and leq_Ref (v1, own1) (v2, own2) =
    if own1 = own2 then leq_V v1 v2 else false

  and eq_Ref (v1, own1) (v2, own2) =
    if own1 = own2 then leq_V v1 v2 else false

  and wid_Ref ((v1, own1) as ref1) (v2, own2) =
    if own1 = own2 then (wid_V v1 v2, own1) else ref1
    (* TODO: if two ownership not equal, return what *)

  and arrow_Ref var (v, own) v' = (arrow_V var v v', own)

  and stren_Ref (v, own) ae = (stren_V v ae, own)

  and proj_Ref (v, own) vars = (proj_V v vars, own)

  and forget_Ref var (v, own) = (forget_V var v, own)

  and bot_shape_Ref (v, own) = (bot_shape_V v, own)

  and replace_Ref (v, own) var x = (replace_V v var x, own)
  ...
  and transfer_Ref action (v1, own1) (v2, own2) =
    if action = "deref" then ((v1, 0.5), (v1, 0.5))
    else if (* alias *)
            eq_V v1 v2 then ((v1, 1.0), (v2, 0.0))
    else ((v1, 1.0), (v1, 0.0))

  (* *************************************** ** Abstract domain for Execution
     Map ** *************************************** *)
  ...
  let wid_M (m1 : exec_map_t) (m2 : exec_map_t) : exec_map_t =
    NodeMap.union
      (fun n v1 v2 ->
        (* ( let l = get_label_snode n in if !debug then begin Format.printf
           "\n<=== Wid ===> %s\n" l; f Format.std_formatter v1; Format.printf
           "\n<<~~~~>> \n"; f Format.std_formatter v2; Format.printf "\n";
  ...
  let leq_M (m1 : exec_map_t) (m2 : exec_map_t) : bool =
    NodeMap.for_all
      (fun n v1 (*untie to node -> value*) ->
        NodeMap.find_opt n m2
        |> Opt.map (fun v2 -> leq_V v1 v2)
        |> Opt.get_or_else (v1 = Bot) )
      m1
    ...
    NodeMap.for_all
      (fun n v1 (*untie to node -> value*) ->
        ...
        |> Opt.map (fun v2 ->
               ...
               if eq_V v1 v2 then true
               else raise (Pre_Def_Change ("Predefined node changed at " ^ l)) )
        |> Opt.get_or_else (v1 = v1) )
      m1
  ...
  let array_M env m =
    ...
    let t_make =
      ...
      (* make |-> zm: {v:int | v >= 0} -> ex: {v:int | top} -> 
         {v: Int Array (l, e) | len: [| l=zm; zm>=0; |] item: [| l=zm;
         zm>=0; e = ex; |]} *)
      let var_l = "zm" in
      ...
      (*TODO: Make poly*)
      (* let ilen = op_R "i" "0" Ge true llen |> op_R "i" "l" Lt true in let
         rlen = op_R "x" var_e Eq true ilen in *)
      let ary =
        let l, e = ("l", "e") in
        let rl = arrow_R var_l rm rl |> op_R "" l var_l Eq true in
        let re =
          arrow_R var_l rm rl |> op_R "" l var_l Eq true
          |> op_R "" e var_e Eq true
        in
        ((l, e), (rl, re))
      in
      let t =
        let t' =
          Table (construct_table (var_e, var_e) (Relation rm, Ary ary))
        in
        Table (construct_table (var_l, var_l) (Relation rl, t'))
      in
      t
    in
    ...
    let t_len =
      ...
      let ary =
        ...
        ((l, e), (rl, re))
      in
      (* let rl = op_R var_i "0" Ge true llen |> op_R var_i var_len Lt true
         in *)
    ...
    let t_get =
      ...
      let ary =
        ...
        let rl =
          replace_R
            (top_R Plus |> op_R "" "cur_v" "0" Ge true)
            "cur_v" var_len
        in
        let re = top_R Plus in
        ((l, e), (rl, re))
      ...
      let rm =
        top_R Plus
        |> op_R "" "cur_v" "0" Ge true
        |> op_R "" "cur_v" var_len Lt true
      in
      let rr = top_R Plus |> op_R "" "cur_v" "e" Eq true in
      let var_zi = "zi" in
      let t =
        let t' =
          Table (construct_table (var_zi, var_zi) (Relation rm, Relation rr))
        in
        Table (construct_table (var_l, var_l) (Ary ary, t'))
      in
      t
    in
    ...
    let t_set =
      ...
      let t =
        let t' =
          Table (construct_table (var_e, var_e) (Relation rri, Tuple rrr))
        in
        let t'' =
          Table (construct_table (var_zi, var_zi) (Relation rm, t'))
        in
        Table (construct_table (var_l, var_l) (Ary ary, t''))
      in
      t
    in
    let m' =
      m |> NodeMap.add s_make t_make |> NodeMap.add s_len t_len
      |> NodeMap.add s_get t_get |> NodeMap.add s_set t_set
    in
    let env' =
      env
  ...
  let list_M env m =
    ...
    let t_hd =
      (* hd *)
      (* hd |-> zh: { v: 'a List (l, e) | l: [| l>=0; |] e: true }  -> true  *)
      let var_h = "zh" in
      (* let var_i = "i" in *)
      let list =
        let l, e = ("l", "e") in
        let rl =
          replace_R (top_R Plus |> op_R "" "cur_v" "0" Ge true) "cur_v" l
        in
        let ve = Top in
        ((l, e), (rl, ve))
      in
      let tr = Top in
      let t = Table (construct_table (var_h, var_h) (Lst list, tr)) in
      t
    in
    ...
    let t_tl =
      ...
      let list2 =
        ...
        let rl =
          replace_R (top_R Plus |> op_R "" "cur_v" "0" Ge true) "cur_v" l
          |> op_R l var_len "1" Minus true
        in
        (* let re = replace_R (rl |> op_R "" "cur_v" var_e Eq true) "cur_v" e
           in *)
        let ve = Top in
        ((l, e), (rl, ve))
      in
      let t =
        Table (construct_table (var_t, var_t) (Lst list1, Lst list2))
      in
      t
    in
    ...
    let t_cons =
      (* cons *)
      (* cons |-> zc: true -> xs: { v: Int List (l, e) | l: [| l>=0; |] e:
         true } ->  { v: Int List (l1, e1) | l': [| l1=l+1; |] e': e ⊔ zc |]
         } *)
      let var_c = "zc" in
      let ve = Top in
      let var_len, var_e = ("l", "e") in
      ...
      let list2 =
        let l, e = ("l'", "e'") in
        let rl =
          replace_R (top_R Plus |> op_R "" "cur_v" "1" Ge true) "cur_v" l
          |> op_R l var_len "1" Plus true
        in
        let ve = Top in
        ((l, e), (rl, ve))
      in
      let t =
        let t' =
          Table (construct_table (var_l, var_l) (Lst list1, Lst list2))
        in
        Table (construct_table (var_c, var_c) (ve, t'))
      in
      t
    in
    let m' =
      m |> NodeMap.add s_hd t_hd |> NodeMap.add s_len t_len
      |> NodeMap.add s_tl t_tl |> NodeMap.add s_cons t_cons
    in
    let env' =
      env
    ...
    pre_def_func :=
      List.append !pre_def_func
        ["List.hd"; "List.length"; "List.tl"; "List.cons"] ;
    (env', m')

  let pref_M env m =
    let m', env' =
      if VarDefMap.is_empty !pre_vars then (m, env)
      else
        VarDefMap.fold
          (fun var (domain : pre_exp) (m, env) ->
            if contains var "pref" = false then (m, env)
            else
              ...
              let t_var =
                ...
                | {name= n; dtype= Int; left= l; op= bop; right= r} ->
                    let rm =
                      if l = "true" then top_R Plus
                      else top_R Plus |> op_R "" l r bop true
                    in
                    Relation rm
                | {name= n; dtype= Bool; left= l; op= bop; right= r} ->
                    let rm =
                      if l = "true" then init_R_c (Boolean true)
                      else if l = "false" then init_R_c (Boolean false)
                      else top_R Plus |> op_R "" l r bop true
                    in
                    Relation rm
                | {name= n; dtype= Unit; left= l; op= _; right= r} ->
                    if l = "unit" then Tuple []
                      ...
                      raise
                        (Invalid_argument
                           "Expected unit predicate as {v: Unit | unit }" )
              in
  ...
  let construct_predef_pred pre_var var domain =
    ...
    | {name= n; dtype= Unit; left= l; op= _; right= r} ->
        if l = "unit" then Tuple []
          ...
          raise
            (Invalid_argument "Expected unit predicate as {v: Unit | unit }")
end
