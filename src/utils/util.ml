...
module Opt = struct
  ...
  let get_or_else default = function Some x -> x | None -> default

  let lazy_get_or_else f y = function Some x -> x | None -> f y

  let or_else y = function Some x -> Some x | None -> Some y

  let lazy_or_else f y = function Some x -> Some x | None -> f y

  let fold f init = function Some x -> f init x | None -> init
  ...
  let some x = function None -> Some x | o -> o

  let exist = function None -> false | Some _ -> true
end
...
(* l1 <> "l" && contain_var_R l1 rl2 && contain_var_R l2 rl1 then let l', e'
   = fresh_length (), fresh_item () in let lst1' = alpha_rename_Lst lst1 l1
   l' in let lst2' = alpha_rename_Lst lst2 l2 l' in alpha_rename_Lst lst1' e1
   e', alpha_rename_Lst lst2' e2 e' else if *)
let arrange list =
  List.fold_right
    (fun y acc -> List.fold_right (fun x acc -> [x; y] :: acc) list acc)
    list []
...
(** measure accumulated execution time and number of calls to a particular
    function *)
let measure fn arg =
  let start_time =
...
let measure_call (id : string) fn arg =
  ...
  in
  let calls, time =
    if Hashtbl.mem measures id then Hashtbl.find measures id else (0, 0.)
  in
...
let print_measures () =
  ...
  Hashtbl.iter
    (fun id (calls, time) ->
      print_endline
        ( "  " ^ id ^ ": " ^ string_of_int calls ^ " call(s), "
        ^ string_of_float time ^ " s" ) )
    measures
